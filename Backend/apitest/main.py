...
def simulate_with_gdb(code: str, exec_path: str, source_path: str):
    try:
        ...
        for step in steps:
            ...
            # ---- 역행 필터: 같은 함수 안에서만 제한, 함수가 바뀌면(호출/리턴) 역행 허용 ----
            if last_line is not None and last_func is not None:
                if curr_func == last_func and curr_line < last_line:
                    prev_txt = code_lines[last_line - 1]
                    if not (is_loopy(line_text) or is_loopy(prev_txt)):
                        # 루프/블록이 아닌 동일 함수 내 역행 → 스킵
                        continue
            ...
            last_func, last_line = curr_func or last_func, curr_line
            step_counter += 1
...
async def compile_and_analyze(code: str = Form(...), input: str = Form("")):
    ...
    # 2. 컴파일
    try:
        subprocess.run(["gcc", "-g", "-O0", FILENAME, "-o", "a.exe"],
                       cwd=UPLOAD_DIR,
                       check=True,
                       capture_output=True,
                       text=True)
