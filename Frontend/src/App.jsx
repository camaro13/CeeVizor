...
function normalizeStep(raw) {
  ...
  return {
    ...
    // json 파일에서 output이 stdout으로 되어있길래 그것도 지원가능하도록 추가
...
function App() {
  ...
  // 상태 추가 / 메시지창 & 메모리영역 바로실행되는 거 문제 해결을 위한 상태들
  ...
  const openWarn = (msg) => { setModalMsg(msg); setModalOpen(true); }; //경고 모달 열기 함수

  // cmView/steps/stepIndex/code가 바뀔 때마다 안전하게 확장을 만든 뒤 전달 추가
  ...
  function blinkHeapBlocks(prev, curr) {
    const toLabelVal = (block) => {
      if (block && typeof block === 'object' && 'address' in block && 'value' in block) {
        return [block.address, block.value];
      }
      const [k, v] = Object.entries(block || {})[0] || [undefined, undefined];
      return [k, v];
    };

    const prevMap = {};
    (prev || []).forEach(block => {
      const [label, value] = toLabelVal(block);
      if (label !== undefined) prevMap[label] = value;
    });
  ...
  function deletedStackFrames(curr, next) {
    ...
    // 새프레임이 아래에 생기지 않도록 수정 ( curr의 마지막 인덱스부터 삭제)
    const indices = [];
  ...
  function deletedHeapBlocks(curr, next) {
    const labelOf = (block) => {
      if (block && typeof block === 'object' && 'address' in block) return block.address;
      return Object.keys(block || {})[0];
    };
    const nextLabels = new Set((next || []).map(block => labelOf(block)));
  ...
  useEffect(() => {
    ...
    console.log("current output:", newLine) // 제대로 찍히는지 일단 화긴 
  }, [stepIndex, steps]);

  // 전체 실행 핸들러
  // 코드 입력창이 비어있거나 실행할 단계 데이터가 없을 때 경고 메시지 표시
  const handleRun = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const normalized = normalizeSteps(result.timeline || []);
    ...
    // 2) 폴백: 미리 로드한 /steps 결과 사용
    if (!data.length) data = loadedSteps;
  ...
  // 전체, 한줄 버튼 시간 꼬이는 거 수정 코드 
  const handleStepOnce = async () => {
    // ===== /compile 먼저 호출해서 data 확보 =====
    if (!code.trim()) {
      openWarn('코드를 입력한 뒤 실행해주세요.');
      return;
    }
    ...
    if (!response.ok) {
      ...
      openWarn(errText || `HTTP ${response.status}`);
      return;
    }

    const result = await response.json();
    const data = normalizeSteps(result.timeline || []);
    if (!data.length) {
      openWarn('실행할 단계 데이터가 없습니다.');
      return;
    }
    ...
    if (!steps.length) {
      ...
      setSteps(data); // 새로 받은 data 기준으로 진행
      setStepIndex(i => {
